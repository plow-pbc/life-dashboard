...
```sh
cp .env.example .env
# Optionally fill in ICAL_URL as a fallback to the pushed calendar feed.
...
|---|---|---|---|
| `ICAL_URL` | no | — | Full private ICS URL. Secret. Used when no fresh pushed feed is available; blank → `/api/ical` answers 502 and, without a stale feed, the client shows its "Can't reach calendar" state. |
...
| `DASHBOARD_TOKEN` | no | — | Bearer token for the remote write APIs (`/api/message`, `/api/calendar`, the texted-photo CRUD `POST`/`DELETE /api/banners`) and the off-box `GET /api/version` verification read. Setting it enables those routes and binds the server on `0.0.0.0` (LAN-reachable). Secret. |
...
One Node process serves the Vite-built React SPA, stores the bearer-pushed calendar feed at `/api/calendar`, and proxies the secret ICS fallback at `/api/ical` with a 60-second in-memory cache and stale-on-failure fallback. The React app prefers a fresh pushed feed, then parses ICS with `ical.js` (recurrence-aware, drops `STATUS:CANCELLED`), then renders a stale pushed feed with its last-updated time, then shows "Can't reach calendar". A successful empty source shows "No upcoming events." It renders at most `NEXT_N` events. The page calls `location.reload()` every `REFRESH_MS` (5 min default) — that, not in-app polling, is the freshness + state-recovery mechanism. Without `DASHBOARD_TOKEN` the server binds loopback only; with it, it binds `0.0.0.0` so remote producers can reach the write APIs. A Host-header allowlist defends loopback callers against DNS rebinding; non-loopback callers may only POST `/api/message` or `/api/calendar`, mutate banners via `POST`/`DELETE /api/banners`, and read `GET /api/version` — all with the bearer (every other read, the banner listing, and the SPA stay loopback) plus the open `/healthz` probe.
...
endpoint (`docs/kiosk-protocol.md`). Storage is a file-backed JSON store
(`data/messages.json`) keyed by **card number**: each POST writes the latest
message for its card slot, so a chatty producer can never evict another card's
content. The kiosk browser fetches `/api/message?card=N` from
